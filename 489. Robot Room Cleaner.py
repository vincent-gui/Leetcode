...
这里的backTrack 其实是变回原来的状态
比如当前(0, 0), 向上走一步变成(-1, 0), 还是向上, 但是到墙壁了, 右侧也是墙壁, 
这个时候会变成(-1, 0) 向下, 走回到(0, 0), 因为(0, 0) 已经走过了, 这个时候需要backtrack
会出现 转180度, 向上, 再走回到(-1, 0), 向下的状态, 这个时候在(-1, 0) 这个点已经扫描过(上, 右, 下)三个方向了, 
下一步就可以直接扫描方向左
